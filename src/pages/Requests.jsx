...
export default function Requests({ user }) {
  ...
  const fetchAll = async () => {
    ...
    setRequests(reqs || [])
    setUserVotes(new Set((votes || []).map(v => v.request_id)))
    setLoading(false)
  }
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Vote for movies you want added. Most voted gets added first.</p>
          </div>
          <button
            onClick={() => { setShowForm(f => !f); setDuplicate(null) }}
            className="bg-purple-600 hover:bg-purple-700 text-white px-5 py-2 rounded-lg font-semibold transition text-sm"
          >
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-800 border border-gray-600 rounded-lg px-4 py-3 text-white placeholder-gray-500 outline-none focus:border-purple-500"
                  required
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-gray-800 border border-gray-600 rounded-lg px-4 py-3 text-white placeholder-gray-500 outline-none focus:border-purple-500 resize-none"
                />
              </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-3">
            {requests.map((req, index) => (
              <div key={req.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5 flex items-center gap-5 hover:border-gray-700 transition">
                {/* Rank */}
                <span className="text-2xl font-bold text-gray-700 w-8 text-center flex-shrink-0">
                  {index + 1}
                </span>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {req.release_year && <span className="text-gray-400 text-sm">({req.release_year})</span>}
                ...
                <button
                  ...
                  className={`flex flex-col items-center px-4 py-3 rounded-xl border transition flex-shrink-0 ${
                    userVotes.has(req.id)
                      ? 'bg-purple-900/50 border-purple-600 text-purple-400'
                      : 'bg-gray-800 border-gray-700 text-gray-300 hover:border-purple-600 hover:text-purple-400'
                  }`}
                >
                  <span className="text-lg leading-none">▲</span>
                  <span className="text-sm font-bold mt-1">{req.vote_count}</span>
                </button>
              </div>
            ))}
          </div>
