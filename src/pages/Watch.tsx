...
import { fetchMovieDetails, fetchMovieBoxSource, getEmbedSources } from '../lib/movie-api'
  ...
  return (
    <div className="aspect-video flex items-center justify-center bg-gray-900 rounded-lg">
      <div className="text-center px-4">
        ...
        <p className="text-gray-500 text-sm">{message || 'None of our sources have this movie yet.'}</p>
      </div>
    </div>
  )
}
...
export default function Watch({ user }: WatchProps) {
  ...
  const fetchMovieData = useCallback(async () => {
    ...
    try {
      ...
      const embedSources = getEmbedSources(tmdbId)

      const mbUrl = await fetchMovieBoxSource(data.title, year)
      const allSources = mbUrl
        ? [{ key: 'moviebox', name: 'MovieBox', embedUrl: mbUrl }, ...embedSources]
        : embedSources

      setSources(allSources)

      if (allSources.length > 0 && !activeSource) {
        setActiveSource(allSources[0].key)
      }
    } catch (err) {
