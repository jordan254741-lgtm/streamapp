...
const STATUS_STYLES: Record<string, string> = {
  ...
  sourcing: 'bg-blue-50 text-blue-800 border border-blue-300',
  available:'bg-green-50 text-green-800 border border-green-300',
    ...
    <Layout user={user} maxWidth="lg" showBack backTo="/browse" backLabel="Browse">
      <div className="flex items-center justify-between mb-6">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold text-warm-900">Movie Requests</h1>
          <p className="text-warm-500 text-xs sm:text-sm mt-1">Vote for movies you want added. Most voted gets added first.</p>
        </div>
        <button
          onClick={() => { setShowForm(f => !f); setDuplicate(null) }}
          className="bg-crimson hover:bg-crimson-hover text-white px-4 sm:px-5 py-2 rounded-lg font-semibold transition text-sm whitespace-nowrap ml-4"
        >
      ...
      {showForm && (
        <div className="bg-card border border-warm-200 rounded-xl p-4 sm:p-6 mb-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              <label className="text-sm text-warm-600 mb-1 block">Movie Title *</label>
              <input
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-sm text-warm-600 mb-1 block">Release Year</label>
                <input
              ...
              <div>
                <label className="text-sm text-warm-600 mb-1 block">Language</label>
                <input
            ...
            <div>
              <label className="text-sm text-warm-600 mb-1 block">Notes (optional)</label>
              <textarea
            ...
            <button type="submit" disabled={submitting}
              className="bg-crimson hover:bg-crimson-hover text-white py-3 rounded-lg font-semibold transition disabled:opacity-50">
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <p className="text-warm-600 text-lg">No requests yet.</p>
          <p className="text-warm-500 text-sm mt-2">Be the first to request a movie!</p>
        </div>
      ) : (
            ...
            <div key={req.id} className="bg-card border border-warm-200 rounded-xl p-4 sm:p-5 flex items-center gap-3 sm:gap-5 hover:border-warm-300 transition">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                  <h3 className="font-semibold text-warm-900 text-sm sm:text-base truncate">{req.title}</h3>
                  {req.release_year && <span className="text-warm-500 text-xs sm:text-sm">({req.release_year})</span>}
                  <span className={`text-xs px-2 py-0.5 rounded-full capitalize ${STATUS_STYLES[req.status] || STATUS_STYLES.pending}`}>
                    {req.status}
                  </span>
                </div>
                {req.language && <p className="text-warm-500 text-xs mt-1">Language: {req.language}</p>}
                {req.notes && <p className="text-warm-500 text-xs mt-1 truncate">{req.notes}</p>}
              </div>
